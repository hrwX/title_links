...
@frappe.whitelist()
def search_widget(doctype, txt, query=None, searchfield=None, start=0, page_length=20, filters=None, filter_fields=None,
...
def build_for_autosuggest(res, doctype, is_query=False):
	results = []
	for r in res:
		r = list(r)
...
def doctypes_with_show_link_field_title():
	doctypes = frappe.get_all("DocType", {"title_field": ["!=", ""], "istable": 0}) + frappe.get_all("Property Setter", {"field_name": "title_field", "value": ["!=", ""]})
